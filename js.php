<?php 
	...
	// Cache this javscript file for 30 days
	$offset = 60*60*24*30;
	$ExpStr = "Expires: ".gmdate("D, d M Y H:i:s",time() + $offset)." GMT";
	header($ExpStr);
	header('Content-Type: application/x-javascript');

	// The length of time, in days, that cookies should remain. Default: 3 months
	$cookietime = 90;

?>

var SendDemocracy = "<?php echo dirname($_SERVER['PHP_SELF']); ?>/democracy.php?demSend=true";
var GetDemocracy = "<?php echo dirname($_SERVER['PHP_SELF']); ?>/democracy.php?demGet=true";
...
function SendVote (the_vote) {
    ...
    if (cookie) {
		...
		date = new Date();
		date.setTime(date.getTime()+(<?php echo $cookietime; ?>*24*60*60*1000));
		var expires = "expires="+date.toGMTString();";"		
		document.cookie = "demVoted_"+poll_id+"="+the_vote+";"+expires+";path=<?php echo dirname(dirname(dirname(dirname($_SERVER['PHP_SELF'])))); ?>;";
